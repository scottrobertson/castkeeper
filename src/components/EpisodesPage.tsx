...
function Tooltip({ children, label }: { children: any; label: string }) {
  return (
...
function StatusDot({ episode }: { episode: StoredEpisode }) {
...
function DayHeading({ label }: { label: string }) {
  return (
    <div class="flex items-center gap-3 mt-5 mb-2 first:mt-3">
...
function GroupedEpisodes({ episodes }: { episodes: StoredEpisode[] }) {
  const elements: any[] = [];
  let currentDay = '';
  ...
  for (const episode of episodes) {
    ...
    elements.push(<EpisodeRow episode={episode} />);
  }
